...
static inline void refill_32(GetBitContext *s, int is_le) {
  ...
  if (is_le)
    s->cache = (uint64_t)AV_RL32(s->buffer + (s->index >> 3)) << s->bits_left |
               s->cache;
  else
...
static inline int check_marker(void *logctx, GetBitContext *s,
                               const char *msg) {
  int bit = get_bits1(s);
  if (!bit)
    printf("Marker bit missing at %d of %d %s\n", get_bits_count(s) - 1,
           s->size_in_bits, msg);
