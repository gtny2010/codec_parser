...
void av_bprint_init(AVBPrint *buf, unsigned size_init, unsigned size_max) {
  unsigned size_auto =
      (char *)buf + sizeof(*buf) - buf->reserved_internal_buffer;
...
void av_bprint_strftime(AVBPrint *buf, const char *fmt, const struct tm *tm) {
  ...
  while (1) {
    ...
    room = !room ? strlen(fmt) + 1 : room <= INT_MAX / 2 ? room * 2 : INT_MAX;
...
void av_bprint_escape(AVBPrint *dstbuf, const char *src,
  ...
  default:
    ...
    for (; *src; src++) {
      ...
      int is_special = is_strictly_special || strchr("'\\", *src) ||
                       (is_ws && (flags & AV_ESCAPE_FLAG_WHITESPACE));

      if (is_strictly_special || (!(flags & AV_ESCAPE_FLAG_STRICT) &&
