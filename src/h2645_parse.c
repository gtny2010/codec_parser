...
#define FIND_FIRST_ZERO                                                        \
  ...
  for (i = 0; i + 1 < length; i += 9) {
    if (!((~AV_RN64(src + i) & (AV_RN64(src + i) - 0x0100010001000101ULL)) &
  ...
  for (i = 0; i + 1 < length; i += 5) {
    if (!((~AV_RN32(src + i) & (AV_RN32(src + i) - 0x01000101U)) & 0x80008080U))
      continue;
    FIND_FIRST_ZERO;
    STARTCODE_TEST;
    i -= 3;
...
static int hevc_parse_nal_header(H2645NAL *nal, void *logctx) {
  ...
  printf("nal_unit_type: %d(%s), nuh_layer_id: %d, temporal_id: %d\n",
...
static int h264_parse_nal_header(H2645NAL *nal, void *logctx) {
  ...
  printf("nal_unit_type: %d(%s), nal_ref_idc: %d\n", nal->type,
         h264_nal_unit_name(nal->type), nal->ref_idc);
...
static void alloc_rbsp_buffer(H2645RBSP *rbsp, unsigned int size, int use_ref) {
  ...
  if (use_ref) {
    rbsp->rbsp_buffer_ref =
        av_buffer_create(rbsp->rbsp_buffer, size, NULL, NULL, 0);
    if (!rbsp->rbsp_buffer_ref)
      goto fail;
  }
...
int ff_h2645_packet_split(H2645Packet *pkt, const uint8_t *buf, int length,
  ...
  while (bytestream2_get_bytes_left(&bc) >= 4) {
    ...
    if (bytestream2_tell(&bc) == next_avc) {
      int i = 0;
      extract_length = get_nalsize(nal_length_size, bc.buffer,
                                   bytestream2_get_bytes_left(&bc), &i, logctx);
      if (extract_length < 0)
      ...
      if (bytestream2_tell(&bc) > next_avc)
        printf("Exceeded next NALFF position, re-syncing.\n");
    ...
    if (pkt->nals_allocated < pkt->nb_nals + 1) {
      ...
      tmp = av_fast_realloc(pkt->nals, &pkt->nal_buffer_size,
                            new_size * sizeof(*pkt->nals));
      if (!tmp)
        return AVERROR(ENOMEM);
      ...
      nal->skipped_bytes_pos_size =
          FFMIN(1024, extract_length / 3 + 1); // initial buffer size
      nal->skipped_bytes_pos = av_malloc_array(nal->skipped_bytes_pos_size,
                                               sizeof(*nal->skipped_bytes_pos));
      if (!nal->skipped_bytes_pos)
        return AVERROR(ENOMEM);
    ...
    if (is_nalff && (extract_length != consumed) && extract_length)
      printf("NALFF: Consumed only %d bytes instead of %d\n", consumed,
             extract_length);
    ...
    if (ret < 0) {
      printf("Invalid NAL unit %d, skipping.\n", nal->type);
      continue;
    }
