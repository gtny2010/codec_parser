...
#define ASSERT_PTHREAD_ABORT(func, ret)                                        \
  do {                                                                         \
    char errbuf[AV_ERROR_MAX_STRING_SIZE] = "";                                \
    av_log(                                                                    \
        NULL, AV_LOG_FATAL, AV_STRINGIFY(func) " failed with error: %s\n",     \
        av_make_error_string(errbuf, AV_ERROR_MAX_STRING_SIZE, AVERROR(ret))); \
    abort();                                                                   \
  } while (0)

#define ASSERT_PTHREAD_NORET(func, ...)                                        \
  do {                                                                         \
...
static inline int strict_pthread_mutex_init(pthread_mutex_t *mutex,
...
static inline int strict_pthread_cond_init(pthread_cond_t *cond,
...
static inline int
strict_pthread_cond_timedwait(pthread_cond_t *cond, pthread_mutex_t *mutex,
...
static inline int strict_pthread_once(pthread_once_t *once_control,
