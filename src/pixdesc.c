...
void av_read_image_line2(void *dst, const uint8_t *data[4],
  ...
  if (flags & AV_PIX_FMT_FLAG_BITSTREAM) {
    ...
    const uint8_t *p =
        data[plane] + y * linesize[plane] + x * step + comp.offset;
    ...
    while (w--) {
      ...
      if (is_8bit)
        ...
        val = flags & AV_PIX_FMT_FLAG_BE ? AV_RB32(p) : AV_RL32(p);
      val = (val >> shift) & mask;
      if (read_pal_component)
        val = data[1][4 * val + c];
      p += step;
...
void av_read_image_line(uint16_t *dst, const uint8_t *data[4],
                        const int linesize[4], const AVPixFmtDescriptor *desc,
                        int x, int y, int c, int w, int read_pal_component) {
  av_read_image_line2(dst, data, linesize, desc, x, y, c, w, read_pal_component,
                      2);
}
...
void av_write_image_line2(const void *src, uint8_t *data[4],
  ...
  if (flags & AV_PIX_FMT_FLAG_BITSTREAM) {
    ...
    int shift = comp.shift;
    uint8_t *p = data[plane] + y * linesize[plane] + x * step + comp.offset;
...
#define BAYER8_DESC_COMMON                                                     \
  .nb_components = 3, .log2_chroma_w = 0, .log2_chroma_h = 0,                  \
  .comp = {                                                                    \
      {0, 1, 0, 0, 2},                                                         \
      {0, 1, 0, 0, 4},                                                         \
      {0, 1, 0, 0, 2},                                                         \
...
static const char *const color_primaries_names[AVCOL_PRI_NB] = {
    [AVCOL_PRI_RESERVED0] = "reserved",  [AVCOL_PRI_BT709] = "bt709",
    [AVCOL_PRI_UNSPECIFIED] = "unknown", [AVCOL_PRI_RESERVED] = "reserved",
    [AVCOL_PRI_BT470M] = "bt470m",       [AVCOL_PRI_BT470BG] = "bt470bg",
    [AVCOL_PRI_SMPTE170M] = "smpte170m", [AVCOL_PRI_SMPTE240M] = "smpte240m",
    [AVCOL_PRI_FILM] = "film",           [AVCOL_PRI_BT2020] = "bt2020",
...
const AVPixFmtDescriptor *av_pix_fmt_desc_next(const AVPixFmtDescriptor *prev) {
  ...
  while (prev - av_pix_fmt_descriptors <
...
int av_pix_fmt_get_chroma_sub_sample(enum AVPixelFormat pix_fmt, int *h_shift,
                                     int *v_shift) {
...
#define FF_COLOR_YUV                                                           \
  2 /**< YUV color space. 16 <= Y <= 235, 16 <= U, V <= 240 */
#define FF_COLOR_YUV_JPEG                                                      \
  3 /**< YUV color space. 0 <= Y <= 255, 0 <= U, V <= 255 */
...
static int get_pix_fmt_score(enum AVPixelFormat dst_pix_fmt,
                             enum AVPixelFormat src_pix_fmt, unsigned *lossp,
                             unsigned consider) {
  ...
  *lossp = loss = 0;
  ...
  for (i = 0; i < nb_components; i++) {
    ...
    if (src_desc->comp[i].depth - 1 > depth_minus1 &&
        (consider & FF_LOSS_DEPTH)) {
      loss |= FF_LOSS_DEPTH;
  ...
  if (consider & FF_LOSS_RESOLUTION) {
    ...
    // don't favor 422 over 420 if downsampling is needed, because 420 has much
    // better support on the decoder side
  ...
  if (consider & FF_LOSS_COLORSPACE)
    switch (dst_color) {
    case FF_COLOR_RGB:
      if (src_color != FF_COLOR_RGB && src_color != FF_COLOR_GRAY)
    ...
    case FF_COLOR_YUV_JPEG:
      if (src_color != FF_COLOR_YUV_JPEG && src_color != FF_COLOR_YUV &&
  ...
  if (loss & FF_LOSS_COLORSPACE)
    score -= (nb_components * 65536) >>
             FFMIN(dst_desc->comp[0].depth - 1, src_desc->comp[0].depth - 1);

  if (dst_color == FF_COLOR_GRAY && src_color != FF_COLOR_GRAY &&
      (consider & FF_LOSS_CHROMA)) {
    loss |= FF_LOSS_CHROMA;
    score -= 2 * 65536;
  }
  if (!pixdesc_has_alpha(dst_desc) &&
      (pixdesc_has_alpha(src_desc) && (consider & FF_LOSS_ALPHA))) {
    loss |= FF_LOSS_ALPHA;
    score -= 65536;
  }
  if (dst_pix_fmt == AV_PIX_FMT_PAL8 && (consider & FF_LOSS_COLORQUANT) &&
      (src_pix_fmt != AV_PIX_FMT_PAL8 &&
       (src_color != FF_COLOR_GRAY ||
        (pixdesc_has_alpha(src_desc) && (consider & FF_LOSS_ALPHA))))) {
...
int av_get_pix_fmt_loss(enum AVPixelFormat dst_pix_fmt,
  ...
  int loss;
  int ret = get_pix_fmt_score(dst_pix_fmt, src_pix_fmt, &loss,
                              has_alpha ? ~0 : ~FF_LOSS_ALPHA);
  if (ret < 0)
    return ret;
  return loss;
}
  ...
  if (!desc1) {
    ...
    if (score1 == score2) {
      if (av_get_padded_bits_per_pixel(desc2) !=
        ...
        dst_pix_fmt = desc2->nb_components < desc1->nb_components
                          ? dst_pix_fmt2
                          : dst_pix_fmt1;
      }
    } else {
...
const char *av_color_range_name(enum AVColorRange range) {
  return (unsigned)range < AVCOL_RANGE_NB ? color_range_names[range] : NULL;
}
...
const char *av_color_primaries_name(enum AVColorPrimaries primaries) {
  return (unsigned)primaries < AVCOL_PRI_NB ? color_primaries_names[primaries]
                                            : NULL;
}
...
const char *
av_color_transfer_name(enum AVColorTransferCharacteristic transfer) {
  return (unsigned)transfer < AVCOL_TRC_NB ? color_transfer_names[transfer]
                                           : NULL;
}
...
const char *av_chroma_location_name(enum AVChromaLocation location) {
  return (unsigned)location < AVCHROMA_LOC_NB ? chroma_location_names[location]
                                              : NULL;
}
