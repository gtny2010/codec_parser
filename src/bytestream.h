...
bytestream2_init(GetByteContext *g, const uint8_t *buf, int buf_size) {
...
static av_always_inline void
bytestream2_init_writer(PutByteContext *p, uint8_t *buf, int buf_size) {
  av_assert0(buf_size >= 0);
  p->buffer = buf;
  p->buffer_start = buf;
...
static av_always_inline unsigned int
bytestream2_put_bufferu(PutByteContext *p, const uint8_t *src,
                        unsigned int size) {
...
static av_always_inline void
bytestream2_set_buffer(PutByteContext *p, const uint8_t c, unsigned int size) {
  int size2;
  if (p->eof)
    return;
  size2 = FFMIN(p->buffer_end - p->buffer, size);
...
static av_always_inline void
bytestream2_set_bufferu(PutByteContext *p, const uint8_t c, unsigned int size) {
...
static av_always_inline unsigned int
bytestream2_copy_bufferu(PutByteContext *p, GetByteContext *g,
                         unsigned int size) {
...
static av_always_inline unsigned int
bytestream2_copy_buffer(PutByteContext *p, GetByteContext *g,
                        unsigned int size) {
  int size2;
...
static av_always_inline void
bytestream_put_buffer(uint8_t **b, const uint8_t *src, unsigned int size) {
  memcpy(*b, src, size);
  (*b) += size;
}
